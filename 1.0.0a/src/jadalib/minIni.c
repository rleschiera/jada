...
static char *save_strncpy(char *dest, char *source, size_t maxlen, enum quote_option option)
{
  size_t d, s;
...
static int getkeystring(INI_FILETYPE *fp, char *Section, char *Key,
 ...
 */
int ini_gets(char *Section, char *Key, char *DefValue,
             char *Buffer, int BufferSize, char *Filename)
{
  INI_FILETYPE fp;
  int ok = 0;
...
static void ini_tempname(char *dest, char *source, int maxlength)
{
  char *p;
...
static enum quote_option check_enquote(char *Value)
{
  char *p;
...
static void writesection(char *LocalBuffer, char *Section, INI_FILETYPE *fp)
{
  char *p;
...
static void writekey(char *LocalBuffer, char *Key, char *Value, INI_FILETYPE *fp)
{
  char *p;
...
static int cache_accum(char *string, int *size, int max)
{
...
static int close_rename(INI_FILETYPE *rfp, INI_FILETYPE *wfp, char *filename, char *buffer)
{
  (void)ini_close(rfp);
  (void)ini_close(wfp);
