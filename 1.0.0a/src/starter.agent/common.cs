...
namespace JADA.Starter
{
    public class Common
    {
        ...
        // Variabili globali
        private static bool myTestMode = false;
        private static string myModuleName = "";
        private static string myBaseURL = "http://jada.test/jada/";
        private static string myCustomer = "test";
        private static string myBaseDir = "C:/jada.test/";
        private static string myIniFile = "";
        private static string myLogFile = "";

        public static void LogMessage(string szMessage)
        {
            if (ExistsDir(myBaseDir + "log"))
            {
        ...
        public static bool ExistsDir(string szDirName)
        {
        ...
        public static bool CheckMD5File(string szFileName, string szMD5string)
        {
            ...
            if (File.Exists(myBaseDir + szFileName))
            {
                using (var reader = new System.IO.FileStream(myBaseDir + szFileName, System.IO.FileMode.Open, System.IO.FileAccess.Read))
                {
                    ...
                    LogMessage("File " + szFileName + ", LocalMD5=" + szLocalMD5 + ", RemoteMD5=" + szRemoteMD5);
        ...
        public static int Init(string szModuleName, string szBaseURL, string szCustomer)
        {
            ...
            if (myTestMode == true)
            {
                MessageBox.Show("ATTENZIONE!!! Sto lavorando in test mode."); 
                MessageBox.Show("Base Dir: " + myBaseDir + ", Base URL: " + myBaseURL + ", Customer: " + myCustomer);
            }
            else
            {
                myBaseDir = "C:/jada/";
